...
pub const Vector4 = struct {
    ...
    pub fn expectEqual(actual: Vector4, expected: Vector4, tolerance: f32) !void {
...
pub const Vector3 = struct {
    ...
    pub fn length2(self: Vector3) f64 {
    ...
    pub fn multScalar(self: Vector3, scalar: f64) Vector3 {
        return .{ .x = self.x * scalar, .y = self.y * scalar, .z = self.z * scalar };
    }

    pub fn divideScalar(self: Vector3, scalar: f64) Vector3 {
        return .{ .x = self.x / scalar, .y = self.y / scalar, .z = self.z / scalar };
    }
